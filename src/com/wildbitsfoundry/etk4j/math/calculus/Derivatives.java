...
public final class Derivatives {
	...
	public static double centeredDifference5Points(UnivariateFunction fn, double x, double h) {
		double num = 8 * (fn.evaluateAt(x + h) - fn.evaluateAt(x - h)) - fn.evaluateAt(x + 2 * h) + fn.evaluateAt(x - 2 * h);
		return num / (12 * h);
	}
	
	public static double centeredDifference7Points(UnivariateFunction fn, double x, double h) {
		double num = 45 * (fn.evaluateAt(x + h) - fn.evaluateAt(x - h)) + 9 *(fn.evaluateAt(x - 2 * h) - fn.evaluateAt(x + 2 * h))
				   + fn.evaluateAt(x + 3 * h) - fn.evaluateAt(x - 3 * h);
		return num / (60 * h);
	}
}
