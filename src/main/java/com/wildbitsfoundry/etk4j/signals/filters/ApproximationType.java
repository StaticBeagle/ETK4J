...
public enum ApproximationType {
	...
	INVERSE_CHEBYSHEV
	{
		...
		ZeroPoleGain buildLowPassPrototype(int n, double ap, double as) {		
			...
			for(int k = 0; k < zeros.length;) {
				Complex zero = Complex.newComplex(0.0, -1.0 / Math.cos(0.5 * Math.PI * (k + 1) * nInv));
				zeros[k++] = zero;
				zeros[k++] = zero.conj();
			}
			double k = computeGain(zeros, poles);
	...
	ELLIPTIC {
		...
		ZeroPoleGain buildLowPassPrototype(int n, double ap, double as) {
			if(n == 1) {
				...
				p[0] = Complex.newComplex(-Math.sqrt(1.0 / (Math.pow(10.0, ap * 0.1) - 1.0)), 0.0);
				double k = -p[0].real();
			...
			for(int i = 0, m = zeros.length - 1; i < n3; ++i, m = m - 2) {
				...
				poles[m] = pole;
				poles[m - 1] = pole.conj();
				Complex zero = Complex.newComplex(0.0, d / ek.get(0));
				zeros[m] = zero;
				zeros[m - 1] = zero.conj();
			}
			if(n0 == 1) {
	...
	static double computeGain(Complex[] zeros, Complex[] poles) {
		// Compute gain k
		Complex knum = Complex.newComplex(1.0, 0.0);
		for(Complex zero : zeros) {
			knum.multiplyEquals(zero.uminus());
		}
		Complex kden = Complex.newComplex(1.0, 0.0);
